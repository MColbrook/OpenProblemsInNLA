...
def compiler_lock_path() -> Path:
    """Keep the shared macOS lock; use the system temporary directory elsewhere."""
    directory = Path("/private/tmp") if sys.platform == "darwin" else Path(tempfile.gettempdir())
    return directory / "nla-lean-compiler.lock"

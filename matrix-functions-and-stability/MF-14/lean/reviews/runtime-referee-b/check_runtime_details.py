...
E = D / 'verification/MF14-repaired-linux-20260919'
...
snapshot = json.loads((D / 'final-review-packets/MF14-v2-instance-alignment/REVIEW-SNAPSHOT.json').read_text())
...
for entry in spec['runs']:
    ...
    assert '{"include":[{"id":"MF-14","project":"matrix-functions-and-stability/MF-14/lean"}]}' in selected
    joblog = (root / f"official-job-{verify['id']}.raw.log").read_text()
    assert 'PASS: fresh Comparator run and all controls.' in joblog
    assert 'lean-MF-14.zip successfully finalized.' in joblog
    checks.append({'kind': entry['kind'], 'run_id': entry['run_id'],
                   ...
                   'official_selected_projects': ['MF-14'],

...
for entry in spec['runs']:
    ...
    assert run['status'] == 'completed', 'Do not freeze mutable evidence from an unfinished run'
    ...
    if spec.get('expected_conclusion') == 'failure':
        assert run['conclusion'] == 'failure'
        assert not receipt_files, 'Failed-run collection expects no successful verification receipt'
        verify_jobs = [j for j in jobs['jobs'] if j['name'].startswith('verify (')]
        assert len(verify_jobs) == 1 and verify_jobs[0]['conclusion'] == 'failure'
        verify_log = (root / f"official-job-{verify_jobs[0]['id']}.raw.log").read_text()
        checked_lines = re.findall(r'git log -1 --format=%H\r?\n[^\n]*?Z ([a-f0-9]{40})\r?\n', verify_log)
        assert len(checked_lines) == 1, 'Require the actual checkout log SHA, not an inferred merge commit'
        checked_sha = checked_lines[0]
        checked_commit_basis = 'Official checkout step git log -1 --format=%H; no success receipt exists'
    else:
        assert len(receipt_files) == 1
        receipt = json.loads(receipt_files[0].read_text())
        checked_sha = receipt['repository_commit']
        checked_commit_basis = 'Successful verification result.json receipt'
    ...
    provenance = {'repository': repository, 'run': run, 'jobs': jobs,
                  ...
                  'checked_commit_basis': checked_commit_basis,

...
Two wholly nonauthor final reviews **approved** the complete original-target proof: [referee A](reviews/final-referee-a/REPORT.md) and [referee B](reviews/final-referee-b/REPORT.md). Both audited the exact source bytes and existing local evidence; neither reran Lean or Comparator. The real GitHub Linux Comparator/kernel/sandbox checks also **passed**, with a separate independent evidence audit. The complete original negative answer is Lean verified. See `STATE.json`, `formalization.yaml`, and `PACKAGING-NEXT-STEPS.md` for the exact verification record.
...
The toolchain and dependencies are pinned in `lean-toolchain`, `lakefile.toml`, and `lake-manifest.json`. `Solution` is the sole default target. `Challenge` is isolated and is not imported by the implementation. Final Comparator execution uses the repository's pinned non-root Linux workflow; the actual run URLs and exact proof commit are recorded below.
...
The immutable proof commit is [c89c859a6b21c399a99f5e9f5fb2b7f472c96657](https://github.com/sgstepaniants/OpenProblemsInNLA/tree/c89c859a6b21c399a99f5e9f5fb2b7f472c96657/matrix-functions-and-stability/MF-14/lean). The [sgstepaniants run 35433896875](https://github.com/sgstepaniants/OpenProblemsInNLA/actions/runs/35433896875) and [ajt60gaibb run 35433898434](https://github.com/ajt60gaibb/OpenProblemsInNLA/actions/runs/35433898434) both passed all 25 exact contracts. Each selected only MF-14 and matched all 536 published project inputs to that revision. The upstream run used GitHub's separately identified synthetic merge commit, with the same proof inputs.
...
```sh
tools/lean/bootstrap.sh /tmp/mf14-lean-tools
tools/lean/verify.sh matrix-functions-and-stability/MF-14/lean /tmp/mf14-lean-tools
```

[Upstream PR #305](https://github.com/ajt60gaibb/OpenProblemsInNLA/pull/305) requests inclusion in main. This is one formalized existing resolution; no new mathematical resolution is counted. The copied original audit script records its recovery layout and is not a replacement for the repository verifier.

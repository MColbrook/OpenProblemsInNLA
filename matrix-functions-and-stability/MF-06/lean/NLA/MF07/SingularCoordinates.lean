...
lemma inverseDiagonalWeights_star {d : ℕ} (σ : Fin d → ℝ) :
  ...
  simpa only [inverseDiagonalWeights, diagonalWeights, Complex.ofReal_inv] using
    diagonalWeights_star (fun i => (σ i)⁻¹)

lemma ordered_positive_gram_coordinates {d : ℕ} (T : Square d) (hT : T.det ≠ 0) :
    ∃ (Q : Square d) (lam : Fin d → ℝ), IsUnitary Q ∧ (∀ i, 0 < lam i) ∧
      Antitone lam ∧ (T * T.conjTranspose) * Q = Q * diagonalWeights lam := by
  classical
  ...
  have hHQ : H * Q = Q * diagonalWeights lam := by
    change H * Q = Q * Matrix.diagonal (fun i => (lam i : ℂ))
    ext i j
    rw [Matrix.mul_diagonal, Matrix.mul_apply]
    have he : applyMatrix H (b j) = (lam j : ℂ) • b j :=
      hS.apply_eigenvectorBasis hdim j
    have hi := congrArg (fun x : EuclideanVector d => x i) he
    simpa [applyMatrix_coordinate, Q, columnMatrix, PiLp.smul_apply, mul_comm] using hi
  have hconj : Q.conjTranspose * H * Q = diagonalWeights lam := by
    calc
      Q.conjTranspose * H * Q = Q.conjTranspose * (H * Q) := mul_assoc _ _ _
      _ = Q.conjTranspose * (Q * diagonalWeights lam) := by rw [hHQ]
      _ = (Q.conjTranspose * Q) * diagonalWeights lam := (mul_assoc _ _ _).symm
      _ = diagonalWeights lam := by rw [hQ.1, one_mul]
  ...
  have hdiag : (diagonalWeights lam).PosDef := by
    rw [← hconj]
  ...
  have hLam : ∀ i, 0 < lam i := by
    ...
    simpa using hi
  exact ⟨Q, lam, hQ, hLam, hS.eigenvalues_antitone hdim, hHQ⟩
...
lemma exists_singular_coordinates {d : ℕ} (T : Square d) (hT : T.det ≠ 0) :
  ...
  obtain ⟨Q, lam, hQ, hLam, hmono, hHQ⟩ := ordered_positive_gram_coordinates T hT
  let σ : Fin d → ℝ := fun i => Real.sqrt (lam i)
  have hσ : ∀ i, 0 < σ i := fun i => Real.sqrt_pos.mpr (hLam i)
  have hσmono : Antitone σ := fun i j hij => Real.sqrt_le_sqrt (hmono hij)
  have hD : diagonalWeights σ * diagonalWeights σ = diagonalWeights lam := by
    ext i j
    by_cases hij : i = j
    · subst j
      have hi := congrArg Complex.ofReal (Real.mul_self_sqrt (hLam i).le)
  ...
  have hR : IsUnitary R := ⟨hRleft, mul_eq_one_comm.mp hRleft⟩

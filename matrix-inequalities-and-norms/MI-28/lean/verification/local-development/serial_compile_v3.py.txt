...
for mod in sys.argv[3:]:
    ...
    visit(mod)
registry = []
for f in sorted((ROOT / 'runs').glob('development-*/RECEIPT.json')):
...
f = ROOT / 'MI04-QUADRATIC-4096.json'
j = json.loads(f.read_text())
assert j['exit_code'] == 0
registry.append((f, sha(f), {'source_inputs': json.loads((ROOT / 'ASSEMBLY-01.json').read_text())['sources']},
                 dict(j, module='NLA.MI04.Quadratic')))
...
receipt = {'scope': 'macOS serial development only; no Comparator, isolation controls, publication or whole-problem acceptance',
